// ==========================================
        ...
        // Choose a construct builder based on POI type + faction
        function classifyPOI(poi) {
            const t = (poi.type || '').toLowerCase();
            const f = (poi.faction && poi.faction.id) || '';
            // Horror first — Lovecraftian / Abyss / Soul Repository
            if (t.includes('lovecraftian') || t.includes('horror') || t.includes('nightmare') || f === 'abyss_followers' || f === 'unholy_ones') return 'horror';
            // Vampiric / gothic
            if (t.includes('vampiric') || t.includes('court') || f === 'vamperica') return 'gothic';
            // Floating city / hub / sanctuary
            if (t.includes('city') || t.includes('hub') || t.includes('sanctuary') || t.includes('base') || t.includes('capital')) return 'city';
            // Media / nexus / broadcast / construct (spire-shape)
            if (t.includes('nexus') || t.includes('media') || t.includes('broadcast') || t.includes('construct')) return 'spire';
            // Magical / arcane nebula
            if (t.includes('magical') || t.includes('anomaly') || t.includes('arcane') || f === 'magic_whisperers') return 'nebula';
            // Repository / soul / vault / relic / shattered
            if (t.includes('repository') || t.includes('vault') || t.includes('relic') || t.includes('shattered') || t.includes('soul')) return 'relic';
            // Plane / realm = planet (default)
            if (t.includes('plane') || t.includes('realm')) return 'planet';
            return 'planet';
        }
