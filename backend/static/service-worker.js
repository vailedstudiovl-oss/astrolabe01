...
const VERSION       = 'v32-2026-05-24-cinematics-biomes';
...
const SHELL_ASSETS = [
  ...
  '/api/static/js/skill_tree_ui.js',
  '/api/static/js/cinematic_overlay.js',
  '/api/static/cinematic_drop_pod.mp4',
  '/api/static/cinematic_reality_entry.mp4',
...
self.addEventListener('fetch', (event) => {
  ...
  // Death's Ship + Lore archive + main_menu HTML — network-first too, so
  // edits to the 2D game / lore archive ship promptly without stale-cache traps.
  if (url.pathname === '/api/deaths-ship' || url.pathname === '/api/lore') {
    event.respondWith(networkFirst(request, SHELL_CACHE));
    return;
  }
