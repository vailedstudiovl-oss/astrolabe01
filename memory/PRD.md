...
- `launcher_v2.html`   — v2-branded boot installer, points at chunks_v2/.
- `chunks_v2/*`        — generated by `split_astrolabe_v2.py`.
- Server routes: `/api/astrolabe-game-v2` and `/api/astrolabe-v2`.
- `main_menu.html` now defaults to `/api/astrolabe-game-v2`.

### Live Intel Ticker (NEW)
The Astrolabe Intel Feed now merges 3 sources every 90 seconds:
1. `/api/lore/recent`  — community wanderer submissions
2. `/api/lore/canon`   — canon-derived ticker snippets (NEW endpoint)
3. `INTEL_TEMPLATES`   — static flavor fallback

### Service-worker policy
- VERSION → `v6-2026-02-v2merge` (forces cache refresh on next install)
- HTML pages /api/astrolabe, /api/deaths-ship, /api/lore now use
  network-first to avoid stale-cache traps that broke Death's Ship loading
